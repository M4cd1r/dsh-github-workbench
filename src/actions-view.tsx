...
export function ActionsView({ ghRef, visible, onCount }: { ghRef: GhRef; visible: boolean; onCount: (n: number) => void }): ReactNode {
  ...
  async function cancel(run: api.GhRun): Promise<void> {
    if (!(await ui.confirm({
      ...
      confirmText: t('actions.cancelText'), danger: true,
    }))) return;
    try {
    ...
    <div className="gw-colpane" style={{ flex: 1, minHeight: 0, display: 'flex' }}>
      <div className="gw-toolbar">
        <span className="gw-open-count">
          {t('actions.recent', { active: active > 0 ? t('actions.active', { count: active }) : '' })}
        </span>
        <button className="gw-btn" onClick={() => reload(false)}><GwIcon name="refresh" size={11} />{t('actions.refresh')}</button>
      </div>
      <div className="gw-list">
        {error && <ErrorBox msg={error} onRetry={reload} />}
        {!error && !runs && <Loading />}
        {runs?.length === 0 && <Empty>{t('empty.noWorkflowRuns')}</Empty>}
        {runs?.map((run) => {
              ...
              <span className="gw-rowmain">
                ...
                <span className="gw-rowsub">
                  ...
                  {run.status === 'completed'
                    ? t('actions.duration', { duration: fmtDuration(Math.max(0, dur)) })
                    : ` · ${run.status === 'queued' ? t('actions.queued') : t('actions.running')}(#${run.run_attempt})`}
                </span>
                <span className="gw-hoverbar" onClick={(e) => e.stopPropagation()}>
                  <button className="gw-btn" onClick={() => rerun(run)}>
                    <GwIcon name="refresh" size={11} />{t('actions.rerun')}
                  </button>
                  {isActive && (
                    <button className="gw-btn danger" onClick={() => cancel(run)}>{t('actions.cancel')}</button>
                  )}
                  <a className="gw-btn" href={run.html_url} target="_blank" rel="noreferrer">
                    <GwIcon name="external-link" size={11} />{t('actions.openOriginal')}
                  </a>
                </span>
              </span>
